...
const AdminSidebar = ({ handleClose }) => {
  ...
  return (
    <div className="flex flex-col text-xl h-full text-white">
      {menu.map((item, index) => (
        <div key={index} onClick={() => handleNavigate(item)}>
          <div className="px-5 flex items-center gap-5 cursor-pointer py-5 hover:bg-gray-700">
            {item.icon}
            <span>{item.title}</span>
          </div>
          {index !== menu.length - 1 && <Divider sx={{ bgcolor: "gray" }} />}
        </div>
      ))}
    </div>
  );
